                    ...
                    while (sellerMenuInProgress)
                    {
                        ...
                        else if (selectedMenuSeller == "Stan Magazynu")
                        {
                            ...
                            while (magazineInProgress)
                            {
                                ...
                                kreska(100);
                                string selectedMagazineStateItem = drawMenu(magazineMenu, 30, 0);
                                ...
                                if (selectedMagazineStateItem == "Eksportuj CSV")
                                {
                                    try
                                    {
                                        string projectPath = Directory.GetCurrentDirectory();
                                        ...
                                        var csvFilePath = Path.Combine(mainFolder.FullName, "CSV/export.csv");
                                        var sb = new StringBuilder();

                                        sb.AppendLine($"Id;Numer seryjny;Nazwa;Ilosc;Cena;Rozmiar;Plec;");
                                        foreach (var product in products)
                                        {
                                            var newLine = $"{product.Id};{product.SerialNumber};{product.Name};{product.Amount};" +
                                                $"{product.Price};{product.Size};{product.Sex};";
                                ...
                                if (selectedMagazineStateItem == "Importuj CSV")
                                {
                                    try
                                    {
                                        string projectPath = Directory.GetCurrentDirectory();
                                        ...
                                        var csvFilePath = Path.Combine(mainFolder.FullName, "CSV/import.csv");
                                        ...
                                        foreach (var line in dataLines)
                                        {
                                            ...
                                            int.TryParse(values[2], out var amount);
                                            product.Amount = amount;
                                            double.TryParse(values[3], out var price);
                                            product.Price = price;
                                            importedProducts.Add(product);
                                        }

                                        productRepository.ImportProducts(importedProducts);
                                        products = productRepository.GetAllProducts();
                                    }
                                    catch (Exception ex)
                                    {
